...
def thong_ke():
    ...
    if selected_month:
        try:
            month_date = datetime.strptime(f"{selected_month}-01", "%Y-%m-%d")
        except ValueError:
            return jsonify({"success": False, "message": "Tháng không hợp lệ."}), 400
    ...
    cursor.execute("""
        SELECT TOP 5 p.MaSanPham, p.TenSanPham,
               ISNULL(SUM(CASE WHEN d.NgayDatHang BETWEEN ? AND ? THEN c.SoLuong ELSE 0 END), 0) AS SoLuong
        FROM SanPham p
        LEFT JOIN ChiTietDonHang c ON p.MaSanPham = c.MaSanPham
        LEFT JOIN DonHang d ON c.MaDonHang = d.MaDonHang
        GROUP BY p.MaSanPham, p.TenSanPham
        ORDER BY SoLuong ASC, p.TenSanPham
    """, (start_dt, end_dt))

    slow_selling = [{"TenSanPham": row[1], "SoLuong": row[2]} for row in cursor.fetchall()]
    ...
    return jsonify({
        ...
        "slowSellingProducts": slow_selling
    })
